...
def create_TDA(H):
    ...
    TDA_avps.append(encodeAVP('Result-Code', 2001))
    # Create message header (empty)
    TDA=HDRItem()
    # Set command code
    TDA.cmd=H.cmd
...
if __name__ == "__main__":
    ...
    mType, received, rcvdMsgType = receiveNextDiamMsg(Conn)
    while(received != None):
        if(received != None and checkForDiamMsg("3GPP-MT-Data", rcvdMsgType)):
            processTDR(received, rcvdMsgType)
            break
        else:
            mType, received, rcvdMsgType = receiveNextDiamMsg(Conn)
